...
public class EnemyTurn : MonoBehaviour {
     ...
     *  void PerformAction()
     * DESCRIPTION
     *  This funtion decides which action the enemy will do on their turn. The enemy will first try to take out a player with a basic 
     *  attack. If the player is defeated by a basic attack, the attack will execute. Otherwise, the enemy's inventory will be separated
     *  into weapons and potions. The weapons list will be sorted by their base damage in ascending order. Each of the weapons in order
     *  will see if they can take out a player in one turn. The attack will execute if an attack will take out a friendly player. If 
     *  none can be defeated in one turn, the enemy will take their turn to heal an enemy with less than 50% health. If none are found
     *  with low health, the enemy will try to deal as much damage as possibly by executing an attack with their strongest weapon. 
     * RETURNS 
     *  None
     */ 
    /**/
     ...
     * DESCRIPTION
     *  This function searches through each of the potential targets. If the damage calculated were to cause the target to lose all
     *  of their health, this funciton will execute that attack using the weapon specifeid by a_weapon or a basic attack if no weapon
     *  is specified. If an attack was executed, the function will return true. If not, false is returned.  
     * RETURNS
     ...
     * DESCRIPTION
     *  This finds the target which will recieve the most damage from an attack and execute that attack. 
     * RETURNS
     *  None
     */
    /**/
     ...
     * DESCRIPTION
     *  Finds all the entities in turn order that are enemies and returns an array of thefound entities. 
     * RETURNS
     ...
     *      a_weapon --> the weapon that will be used in calculateion. A basic attack is used if no weapon is given. 
     * DESCRIPION
     *  This calculates the damage each target will take with the selected weapon. The base damage is set to 5 if no weapon is 
     *  specified. 
     * RETURNS 
